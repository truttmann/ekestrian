...
class TranslateTable 
{
    ...
    * @return resultSet
    */
    public function fetchAll($where = array(), $order = "code")
    {
    ...
    * @return resultSet
    */
    public function fetchAllPaginate($filters = array(), $order = "code")
    {
        $select = new Select($this->tableGateway->getTable());
        $select->join('langue', 'langue.langue_id = translate.langue_id', array("libelle"), "inner");
    ...
    */
    public function fetchOne($code, $langue_id){
        $rowset = $this->tableGateway->select(function(Select $select) use ($code, $langue_id) {
            $select->where(array('translate.code' => $code, "translate.langue_id" => $langue_id));
            $select->join("langue", "langue.langue_id = translate.langue_id", array("libelle"), "inner");
        });
        $row = $rowset->current();
        if (!$row) {
    ...
    */
    public function fetchOneById($id){
        $rowset = $this->tableGateway->select(function(Select $select) use ($id) {
            $select->where(array('translate_id' => $id));
            $select->join("langue", "langue.langue_id = translate.langue_id", array("libelle"), "inner");
        });
        $row = $rowset->current();
        if (!$row) {
    ...
    * @return User
    */
    public function save(Translate $translate)
    {
        $data = array(
            "code" => $translate->code,
            "langue_id" => (int)$translate->langue_id,
            "value" => $translate->value
        );

        $id = (int)$translate->translate_id;
        if ($id == 0) {
            $this->tableGateway->insert($data);
            $translate->translate_id = $this->tableGateway->getLastInsertValue();
        } else {
            if ($this->fetchOne($translate->code, $translate->langue_id)) {
                $this->tableGateway->update($data, array('translate_id' => $id));
            } else {
    ...
    public function delete(Translate $translate) {
        ...
        $this->tableGateway->delete(array('translate_id' => $translate->translate_id));
        return ;
    }
