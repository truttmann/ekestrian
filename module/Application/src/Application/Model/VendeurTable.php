...
class VendeurTable 
{
    ...
    */
    public function fetchOne($id){
        $id  = (int) $id;
        $rowset = $this->tableGateway->select(array('vendeur_id' => $id));
        $row = $rowset->current();
        if (!$row) {
    ...
    * @return User
    */
    public function save(Vendeur $vendeur)
    {
        $data = array(
            "civility" => $vendeur->civility,
            "firstname" => $vendeur->firstname,
            "lastname" => $vendeur->lastname,
            "email" => $vendeur->email,
            "birthday" => $vendeur->birthday,
            "country_id" => $vendeur->country_id,
            "status" => $vendeur->status
        );

        $id = (int)$vendeur->vendeur_id;
        if ($id == 0) {
            $this->tableGateway->insert($data);
            $vendeur->vendeur_id = $this->tableGateway->getLastInsertValue();
        } else {
            if ($this->fetchOne($id)) {
                $this->tableGateway->update($data, array('vendeur_id' => $id));
            } else {
    ...
    public function delete(Vendeur $vendeur) {
        $this->tableGateway->delete(array('vendeur_id' => $vendeur->vendeur_id));
        return ;
    }
